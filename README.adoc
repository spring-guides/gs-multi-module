...
will have have a library jar and a main application that uses the library. You could also
...
Consequently, you need to create two child projects under a root project.
...
image::images/initializr-library.png[]

NOTE: The preceding image shows the Initializr with Maven chosen as the build tool. You
can also use Gradle. It also shows values of `com.example` and `multi-module-library`
...
image::images/initializr-application.png[]

NOTE: The preceding image shows the Initializr with Maven chosen as the build tool. You
can also use Gradle. It also shows values of `com.example` and `multi-module-application`
as the Group and Artifact, respectively. You will use those values throughout the rest of
this sample.
...
[source,java]
----
implementation files('../library/target/multi-module-library-0.0.1-SNAPSHOT.jar')
----
